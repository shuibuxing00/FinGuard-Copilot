...
def ingest_dataframes_to_splunk(
  ...
  investigation SPL can query real Splunk data instead of mock DataFrames.
    ...
    index = svc.indexes[target_index]
    counts = {"users": 0, "transactions": 0, "devices": 0}

    for _, row in users_df.iterrows():
        event = _prepare_user_event(row)
        index.submit(
            json.dumps(event, default=str),
            sourcetype=SOURCETYPE_USERS,
            host="finguard-copilot",
        )
        counts["users"] += 1

    for _, row in transactions_df.iterrows():
        event = _prepare_transaction_event(row)
        index.submit(
            json.dumps(event, default=str),
            sourcetype=SOURCETYPE_TRANSACTIONS,
            host="finguard-copilot",
        )
        counts["transactions"] += 1

    for _, row in devices_df.iterrows():
        event = _prepare_device_event(row)
        index.submit(
            json.dumps(event, default=str),
            sourcetype=SOURCETYPE_DEVICES,
            host="finguard-copilot",
        )
        counts["devices"] += 1
...
def ingest_from_session(
    ...
    try:
        return ingest_dataframes_to_splunk(users_df, transactions_df, devices_df)
